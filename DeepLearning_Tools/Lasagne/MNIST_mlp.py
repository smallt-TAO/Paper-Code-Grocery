...
"""
Usage example employing Lasagne for digit recognition using the MNIST data set.
...
# This is just some way of getting the MNIST data set from an online location
...
def load_data_set():
    ...
    def load_mnist_images(filename):
        ...
        # -1 stand for the all the data cover.
...
def build_mlp(input_var=None):
    ...
    # (unspecified batch size, 1 channel, 28 rows and 28 columns) and
...
def build_custom_mlp(input_var=None, depth=2, width=800, drop_input=.2, drop_hidden=.5):
    ...
    for _ in range(depth):
        network = lasagne.layers.DenseLayer(network, num_units=width, nonlinearity=nonlin)
        if drop_hidden:
...
def build_cnn(input_var=None):
    ...
    network = lasagne.layers.DenseLayer(
            lasagne.layers.dropout(network, p=.5),
            num_units=10,  # num_units stand for the num of diverse.
...
# data is available as numpy arrays. For big data sets, you could load numpy
# arrays as memory-mapped files (np.load(..., mmap_mode='r')), or write your
# own custom data iteration function. For small data sets, you can also copy
