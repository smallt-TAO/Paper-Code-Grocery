...
And more, we study the message leakage in this structure.
...
class AdversarialCrypto(object):
  ...
  def message_leakage(self, tensor_list):
    # print(tensor_list)
    tensor_part = tf.slice(tensor_list, [0, 0], [-1, 8])
    return tensor_part
  ...
  def __init__(self):
    ...
    # message leakage
    in_e = self.message_leakage(in_k)
    eve_out = self.model('eve', encrypted, in_e)
