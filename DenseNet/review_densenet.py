...
def load_one_data(file):
    batch = unpickle(file)
    data = batch[b'data']
    labels = batch[b'labels']
    print("Loading data{}:{}".format(file, len(data)))
    return data, labels


def load_data(files, data_dir, label_count):
    data, labels = load_one_data(data_dir + '/' + files[0])
    for f in files[1:]:
        data_n, labels_n = load_one_data(data_dir + '/' + f)
