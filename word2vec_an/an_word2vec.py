...
class Word2Vec(utils.SaveLoad):
    ...
    @staticmethod
    def counter_antonym():
        antonym = ['good', 'bad', 'come', 'go', 'up', 'down', 'back', 'white', 'less', 'more', 
                  'big', 'small', ]
        return antonym
    ...
    def create_binary_tree(self):
        ...
        heap1 = self.counter_antonym()

        logger.info("pre trained %i antonym words" % len(heap1))
        heap1 = [self.vocab[i] for i in heap1]
        heap = self.vocab.values()

        heap = [i for i in heap if i not in heap1]
        heapq.heapify(heap)
        for i in xrange(len(self.vocab) - 1 - len(heap1)):
            min1, min2 = heapq.heappop(heap), heapq.heappop(heap)
            heapq.heappush(heap, Vocab(count=min1.count + min2.count, index=i + len(self.vocab), left=min1, right=min2))
        """
        heap += heap1
        heapq.heapify(heap)
            for i in xrange(len(heap1)):
                min1, min2 = heapq.heappop(heap), heapq.heappop(heap)
                heapq.heappush(heap, Vocab(count=min1.count + min2.count, index=i + 2*len(self.vocab) - len(heap1) - 1, left=min1, right=min2))
        """
        # pre-handle the antonym word. 
        j = 0
        for i in xrange(0, len(heap1), 2):
            min1, min2 = heap1[i], heap1[i + 1]
            heapq.heappush(heap, Vocab(count=min1.count + min2.count, index=j + 2*len(self.vocab) - len(heap1) - 1, left=min1, right=min2))
            j += 1

        # handle the all word.
        for i in xrange(int(len(heap1) / 2)):
            min1, min2 = heapq.heappop(heap), heapq.heappop(heap)
            heapq.heappush(heap, Vocab(count=min1.count + min2.count, index=i + 2*len(self.vocab) - int(len(heap1)/2) - 1, left=min1, right=min2))
...
# Example: ./word2vec.py ../data/text8 ../c/questions-words.txt ../data/text8
if __name__ == "__main__":
    ...
    model = Word2Vec(Text8Corpus(infile), size=200, window=5, min_count=5, workers=4)

    if len(sys.argv) > 3:
        outfile = sys.argv[3]
        model.save(outfile + '.antonym.model')
        model.save_word2vec_format(outfile + '.antonym.model.bin', binary=True)
        model.save_word2vec_format(outfile + '.antonym.model.txt', binary=False)
