...
parser.add_argument('--batch_size', type=int, default=32, help='no comment')
...
parser.add_argument('--equal_split', type=bool, default=True, help='use trainEqual.csv and valEqual.csv')
...
if args.equal_split:
    train_listfile = open("/mnt/data/trainEqual.csv", "r")
    test_listfile = open("/mnt/data/valEqaul.csv", "r")
else:
    train_listfile = open("/mnt/data/trainingDataNew.csv", "r")
    test_listfile = open("/mnt/data/alDataNew.csv", "r")
...
args_dict = dict(args._get_kwargs())  # key words for arguments dict.
args_dict['train_list_raw'] = train_list_raw
args_dict['test_list_raw'] = test_list_raw
args_dict['png_folder'] = "/mnt/data/image/"
...
if args.mode == 'train':
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        state_name = '%s.epoch%d.test%.5f.state' % (network_name, epoch, test_loss)
