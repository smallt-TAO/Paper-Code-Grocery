...
for iter, line in enumerate(file.readlines()[1:]):
    # first line of trainingData.csv is header(only for trainingData.csv)
    filepath = line.split(',')[0]
    filename = filepath[:-4]
    wavfile = 'tmp.wav'
    os.system('mpg123 -w ' + wavfile + ' /mnt/TrainingData/' + filepath)
    for augmentIdx in range(0, 1):
        alpha = np.random.uniform(0.9, 1.1)
        offset = np.random.randint(20)
        plotstft(wavfile, channel=0, name='/mnt/data/image/'+filename+'.'+str(augmentIdx)+'.png', alpha=alpha, offset=offset)
