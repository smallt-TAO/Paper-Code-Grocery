...
import sys
sys.path.insert(0, '/export/App/lvlintao/Attention-OCR/dataset/')
import dict0
word_list = dict0.make_title()
from itertools import izip
list_word = dict(izip(word_list.itervalues(), word_list.iterkeys()))
...
class Model(object):
    ...
    def __init__(self,
        ...
        # load data
        if phase == 'train':
            print("******************data_base_dir", data_base_dir, data_path)
            self.s_gen = DataGen(
        ...
        logging.info('path dir: %s' % data_path)
            ...
            for batch in self.s_gen.gen(self.batch_size):
                ...
                for idx, output, ground in zip(range(len(grounds)), step_outputs, grounds):
                    ...
                    if distance_loaded:
                        ...
                        for i in output_valid:
                            if i != 2:
                                print(list_word[i])
                        print("ground_valid", ground_valid)
                ...
                for epoch in range(self.num_epoch):
                   ...
                   for i,batch in enumerate(self.s_gen.gen(self.batch_size)):
                        ...
                        if current_step % self.steps_per_checkpoint == 0:
                            ...
                            if not self.forward_only:
                                checkpoint_path = os.path.join(self.model_dir, "haha.ckpt")
    ...
    def visualize_attention(self, filename, attentions, output_valid, ground_valid, flag_incorrect, real_len):
        ...
        with open(os.path.join(output_dir, 'word.txt'), 'w') as fword:
            fword.write(' '.join([str(c - 3) for c in ground_valid]) + '\n')
            fword.write(' '.join([str(c - 3) for c in output_valid]))
